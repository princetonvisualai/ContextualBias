...
def main():
    ...
    for img_name in labels_dict.keys():
        idx_list = list(np.nonzero(labels_dict[img_name]))
        for idx in idx_list:
            label = int(idx[0])
    ...
    if False:
        ...
        for b in range(arg['nclasses']):
            ...
            print('\nb {}({}), c {}({}), bias {}, co-occur {}, exclusive {}, co-occurence frequency {}'.format(onehot_to_humanlabels[b], b, onehot_to_humanlabels[c], c, biases_b[c], len(co_occur), len(imgs_b)-len(co_occur), len(co_occur)/len(imgs_b)))
